...
function TabJadwal({ child, uid, onSaved }: { child: Child; uid: string; onSaved: (msg: string, switchToTab?: EditTab) => void }) {
  ...
  const [weekendToggling,     setWeekendToggling]      = useState(false)
  ...
  if (loading) return (
    <div className="space-y-4">
      <div className="h-3 w-48 bg-[#F3F4F6] rounded-full animate-pulse" />
      <div className="h-14 bg-[#F3F4F6] rounded-xl animate-pulse" />
      <div className="flex gap-1.5">
        {[1, 2, 3, 4, 5].map(i => <div key={i} className="flex-1 h-9 bg-[#F3F4F6] rounded-xl animate-pulse" />)}
      </div>
      <div className="flex flex-wrap gap-2">
        {[1, 2, 3, 4].map(i => <div key={i} className="h-8 w-20 bg-[#F3F4F6] rounded-lg animate-pulse" />)}
      </div>
    </div>
  )
        ...
        <button
          ...
          onClick={async () => {
            if (weekendEnabled) {
              ...
              setWeekendToggling(true)
              try {
                await updateDoc(childDoc(uid, child.id), { enableWeekend: true })
              } catch {
                setWeekendEnabled(false)
              } finally {
                setWeekendToggling(false)
              }
            }
          }}
          className={`relative w-11 h-6 rounded-full overflow-hidden transition-colors disabled:opacity-60 ${weekendEnabled ? 'bg-[#0095F6]' : 'bg-[#D1D5DB]'}`}
        >
          <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200 flex items-center justify-center ${weekendEnabled ? 'translate-x-5' : 'translate-x-0'}`}>
            {weekendToggling && <span className="w-3 h-3 border-2 border-[#0095F6]/30 border-t-[#0095F6] rounded-full animate-spin" />}
          </span>
        </button>
      </div>
...
function TabTopik({ child, uid, onSaved }: { child: Child; uid: string; onSaved: (msg: string) => void }) {
  ...
  if (loading) return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-1.5">
        {[1, 2, 3, 4].map(i => <div key={i} className="h-7 w-20 bg-[#F3F4F6] rounded-lg animate-pulse" />)}
      </div>
              ...
              <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
                ...
                <div className="flex gap-3">
                  <button onClick={() => setShowSaveConfirm(false)} disabled={saving}
                    className="flex-1 py-2.5 rounded-xl border border-[#DBDBDB] text-[13px] font-semibold hover:bg-[#F5F5F5] disabled:opacity-50 transition-colors">
                    Batal
                  </button>
                  <button onClick={save} disabled={saving}
                    className="flex-1 py-2.5 rounded-xl bg-[#0095F6] text-white text-[13px] font-bold hover:bg-[#0074CC] disabled:opacity-50 transition-colors inline-flex items-center justify-center gap-2">
